...
public class ByPest_initiation extends PRA_Home{
	...
	public void By_pest_initiation_pra(String Pestname, String Country_area_at_risk, String Suggested_title_for_pra, String Pra_start_month, String Pra_start_date, String Pra_due_month, String Pra_due_date, String Pra_area) throws InterruptedException
	{
		...
		logger10.log(Status.PASS, "By Pest button got clicked");
		Thread.sleep(3000);
		jse.executeScript("window.scrollBy(0,600)", "");
		
		save_button.click();
		Thread.sleep(2000);
		logger10.log(Status.PASS, "PRA is not saved without filling 4 mandatory information");
		String pra_name_error_text=pra_name_error.getText();
		Thread.sleep(1000);
		logger10.log(Status.PASS,"1 "+pra_name_error_text);
		String demo_error_text=demo_error.getText();
		Thread.sleep(1000);
		logger10.log(Status.PASS, "2 "+demo_error_text);
		String countryatrisk_error_text=countryatrisk_error.getText();
		Thread.sleep(1000);
		logger10.log(Status.PASS, "3 "+countryatrisk_error_text);
		String title_error_text=title_error.getText();
		Thread.sleep(1000);
		logger10.log(Status.PASS, "4 "+title_error_text);
		...
		logger10.log(Status.PASS, count+" Pest search result");
		int j=1;
		for(WebElement search :search_item)
		{
			String search_option=search.getText();
			logger10.log(Status.PASS, j+" "+search_option);
			j++;
		}
		
		pratylenchus.click();
		Thread.sleep(1000);
		logger10.log(Status.PASS, "pratylenchus pest is selected ");
		
		remove.click();
		logger10.log(Status.PASS, "Remove button got clicked and pest got removed");
		Thread.sleep(1000);
		...
		logger10.log(Status.PASS, "pestname_abc got entered in pestname textbox");
		
		add.click();
		logger10.log(Status.PASS, "Add button got clicked");
		Thread.sleep(1000);
		
		
		yes_button_in.click();
		Thread.sleep(1000);
		logger10.log(Status.PASS, "Is this a demo or test PRA-yes button got clicked");
		Select s1=new Select(wd.findElement(By.xpath("//select[@id='CountryAtRisk']")));
		
		s1.selectByVisibleText(Country_area_at_risk);
		Thread.sleep(1000);
		logger10.log(Status.PASS, "Country area at risk-India is selected");
		
		titel_PRA.sendKeys(Suggested_title_for_pra);
		logger10.log(Status.PASS, "Suggested title for PRa is- pratylenchus india");
		Thread.sleep(1000);
		  ...
		  { 
			  ...
			  Thread.sleep(1000);
			  logger10.log(Status.PASS, "start date selected-"+Pra_start_date+" "+Pra_start_month);
		      } 
			  else if(i==2) 
			  { 
				  Thread.sleep(4000); 
				...
				//  WebElement date2=wd.findElement(By.xpath("//td[@class='day' and text()='"+Pra_due_date+"']"));
				  ...
				  Thread.sleep(2000);
				  logger10.log(Status.PASS, "PRA due date selected-"+ Pra_due_date+" "+Pra_due_month);
		...
		Thread.sleep(2000);
		logger10.log(Status.PASS, "PRA area-UK is entered");
		scop.sendKeys("Airlines");
		Thread.sleep(2000);
		logger10.log(Status.PASS, "Define scope of PRA-Airlines is entered ");
		Select s2=new Select(wd.findElement(By.xpath("//select[@id='ReasonForPra']")));
		Thread.sleep(2000);
		s2.selectByVisibleText("Demo or test PRA");
		Thread.sleep(2000);
		logger10.log(Status.PASS, "Reason of PRA-Demo or test PRA is selected");
		Noprevious.click();
		Thread.sleep(2000);
		logger10.log(Status.PASS,"Do previous PRAs exist for this pest- No button got clicked");
		reference_icon.click();
		Thread.sleep(3000);
		logger10.log(Status.PASS, "Reference icon got clicked");
		
	
		wd.switchTo().frame(0);
		Thread.sleep(1000);
		reference_textbox.sendKeys("Text entered -First reference");
		logger10.log(Status.PASS,"Reference got entered");
		Thread.sleep(1000);
		wd.switchTo().defaultContent();
		Thread.sleep(1000);
		save_button.click();
		
		logger10.log(Status.PASS,"Save button got clicked and pest saved succussfully");
		Thread.sleep(3000);
	...
	public void re_edit_initiation_form(String createdpra) throws InterruptedException
	{
		...
		Thread.sleep(2000);
		logger10.log(Status.PASS, "0_automation team link got clicked");
		WebElement created_PRA=wd.findElement(By.linkText(createdpra));
		String pra_name=created_PRA.getText();
		created_PRA.click();
		Thread.sleep(3000);
		logger10.log(Status.PASS, pra_name+" is created successfully in 0_automation team");
		non_editable.click();
		Thread.sleep(2000);
		logger10.log(Status.PASS, "Non editable Gray out area got clicked");
